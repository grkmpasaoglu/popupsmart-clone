...
export async function generateMetadata(
  ...
  const title = helpData[0].attributes.accTitle
  const desc = helpData[0].attributes.accDesc
  ...
  return (
    <main>
      <div className="w-9/12 mx-auto text-xs flex gap-x-4 mb-12">
        ...
        <div><Link href="/help/account" className="font-medium hover:underline">Account</Link></div>
      </div>
      ...
      <div className="container mx-auto max-w-screen-xl md:flex md:justify-center">
        <div className="flex flex-col md:flex-row">
          ...
          <div className="w-4/5 max-w-5xl mx-auto ml-12">
            <div className="text-2xl lg:text-5xl font-bold mb-12">Account</div>
            {helpData.map((help: HelpData) => (
              <div key={help.id} className="ml-5">
                {help.attributes.help_details.data.map((item: HelpDetail) => (
                  <Link href={`/help/account/${item?.id}`} key={item?.id} className="flex items-center">
